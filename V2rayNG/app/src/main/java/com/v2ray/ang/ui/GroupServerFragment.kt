...
class GroupServerFragment : BaseFragment<FragmentGroupServerBinding>(),
    ...
    // Refresh hook: every time the speed-notification coroutine writes bytes
    // to a config, refresh the visible rows so the badge ticks up live.
    private val usageTickListener: () -> Unit = {
        view?.post {
...
//        binding.refreshLayout.setDistanceToTriggerSync((160 * resources.displayMetrics.density).toInt())
    ...
    override fun onResume() {
        ...
        UsageStatsManager.addTickListener(usageTickListener)
        // Refresh once on resume so the badge reflects any traffic that
        // happened while the fragment was paused.
        if (::adapter.isInitialized) adapter.notifyDataSetChanged()
    }

    override fun onPause() {
        super.onPause()
        UsageStatsManager.removeTickListener(usageTickListener)
    }
    ...
    private fun shareServer(guid: String, profile: ProfileItem, position: Int, shareOptions: List<String>, skip: Int) {
        // Build an iOS-style action sheet from the (shareOptions, skip) pair upstream
        // hands us. skip=2 means the row is a custom/policy/chain config — QR and
        // text-share don't apply, so only "Export full" / Edit / Delete render.
        val actions = mutableListOf<ActionSheets.Action>()
        for ((idx, label) in shareOptions.withIndex()) {
            val realIndex = idx + skip
            val (id, icon, destructive, onClick) = when (realIndex) {
                0 -> Quad("qr", R.drawable.ic_action_qrcode, false) { showQRCode(guid) }
                1 -> Quad("copy", R.drawable.ic_copy, false) { share2Clipboard(guid) }
                2 -> Quad("copy_full", R.drawable.ic_description_24dp, false) { shareFullContent(guid) }
                3 -> Quad("edit", R.drawable.ic_edit_24dp, false) { editServer(guid, profile) }
                4 -> Quad("delete", R.drawable.ic_delete_24dp, true) { removeServer(guid, position) }
                else -> Quad("unknown", R.drawable.ic_more_vert_24dp, false) { ownerActivity.toast("else") }
            }
            actions.add(
                ActionSheets.Action(
                    id = id,
        ...
        val title = profile.remarks.ifBlank { getString(R.string.fork_row_actions_title) }
        ActionSheets.show(
            ownerActivity,
            "row_actions",
            title,
            listOf(ActionSheets.Section(actions = actions)),
        )
    }

    /** Local helper to keep the when-branch return shape readable. */
