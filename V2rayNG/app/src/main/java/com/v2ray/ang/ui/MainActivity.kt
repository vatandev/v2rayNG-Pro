    ...
    private fun setupOutageTicker() {
        ...
        if (dotIndex >= 0) {
            ...
            outageBlinkJob = lifecycleScope.launch {
                val greenOn = android.graphics.Color.parseColor("#34C759")
                val greenOff = android.graphics.Color.parseColor("#3334C759")
                val endAt = System.currentTimeMillis() + 10_000L
                var on = true
                while (System.currentTimeMillis() < endAt) {
                    val span = android.text.SpannableString(fullText)
                    span.setSpan(
                        android.text.style.ForegroundColorSpan(if (on) greenOn else greenOff),
                        dotIndex, dotIndex + 1,
