...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        /**
         * Replace the framework's ListPreference dialog (Material AlertDialog with
         * a vertical radio list) with the fork's iOS-style action sheet. Each
         * choice becomes a row in a single section; tapping a row writes the new
         * value via the existing preferenceDataStore and notifies the change
         * listener so all the existing `setOnPreferenceChangeListener` blocks
         * (mux toggles, mode switching, etc.) still fire.
         *
         * Non-ListPreference dialogs (EditText, etc.) fall through to upstream.
         */
        override fun onDisplayPreferenceDialog(preference: Preference) {
            if (preference is ListPreference) {
                val entries = preference.entries ?: emptyArray()
                val values = preference.entryValues ?: emptyArray()
                val current = preference.value
                Log.d("FORK_UI", "settingsDialog open key=${preference.key} entries=${entries.size} current=$current")
                val actions = entries.mapIndexed { idx, entry ->
                    val value = values.getOrNull(idx)?.toString().orEmpty()
                    val isCurrent = value == current
                    ActionSheets.Action(
                        id = preference.key + "/" + value,
                        label = entry,
                        icon = null,
                        destructive = false,
                        selected = isCurrent,
                    ) {
                        if (value != current && preference.callChangeListener(value)) {
                            preference.value = value
                            preference.summary = entry
                            Log.d("FORK_UI", "settingsDialog choose key=${preference.key} value=$value")
                        }
                    }
                }
                ActionSheets.show(
                    activity = requireActivity(),
                    sheetTag = "pref/" + preference.key,
                    title = preference.title,
                    sections = listOf(ActionSheets.Section(actions = actions)),
                )
                return
            }
