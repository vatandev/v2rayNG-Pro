...
object MmkvManager {
    ...
    val trafficStatsStorage: MMKV by lazy { MMKV.mmkvWithID(ID_TRAFFIC_STATS, MMKV.MULTI_PROCESS_MODE) }
    ...
    fun removeServerViaSubid(subscriptionId: String?) {
        ...
        // Fold per-GUID usage rows into the subscription snapshot before
        // wiping them, so the running total survives a sub refresh.
        UsageStatsManager.absorbBeforeSubRefresh(subId, serverList.toList())
