...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun onTestsFinished() {
        viewModelScope.launch(Dispatchers.Default) {
            ...
            if (isQuickTest || MmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_SORT_AFTER_TEST)) {
                sortByTestResults()
            }
            if (isQuickTest) {
                selectBestServer()
    ...
    private fun selectBestServer() {
        ...
        val candidates = if (subId.isEmpty()) {
            MmkvManager.decodeAllServerList()
        } else {
        ...
        val best = candidates
            .mapNotNull { guid ->
                ...
                if (ping <= 0) null else guid to ping
            }
