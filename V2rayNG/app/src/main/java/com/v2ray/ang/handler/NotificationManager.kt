...
object NotificationManager {
    ...
    fun startSpeedNotification(currentConfig: ProfileItem?) {
        ...
        // PREF_SPEED_ENABLED is read every tick — toggling it from Settings
        // while the VPN is up should take effect immediately, not after a
        // VPN restart. activeGuid is also re-read every tick so traffic
        // gets attributed to whichever server the user is on *now*, even
        // if they switched servers mid-session.
        ...
        speedNotificationJob = CoroutineScope(Dispatchers.IO).launch {
            while (isActive) {
                ...
                var proxyUpTotal = 0L
                var proxyDownTotal = 0L
                val text = StringBuilder()
                // Always include the canonical "proxy" tag in addition to whatever
                // outbound tags the current config exposes, so a config with a
                // non-standard outbound name still gets its bytes counted.
                val tagsToQuery = LinkedHashSet<String>().apply {
                    outboundTags?.let { addAll(it) }
                    add(AppConfig.TAG_PROXY)
                }
                tagsToQuery.forEach {
                ...
                val proxyTotal = proxyUpTotal + proxyDownTotal
                ...
                UsageStatsManager.addDelta(activeGuid, proxyDownTotal, proxyUpTotal, sinceLastQueryIn)
                if (proxyDownTotal + proxyUpTotal > 0L) {
                    UsageStatsManager.notifyTick()
                }
